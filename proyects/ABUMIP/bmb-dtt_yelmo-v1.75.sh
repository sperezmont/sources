...
path_restart=${yelmox_path}/${fldr}/restart/
runopt='-r'
...
for i in ${bmbmeth}; do
    ...
    if [ -f "$FILE" ]; then
        ...
        ./runylmox ${runopt} -e ismip6 -n par/yelmo_ismip6_Antarctica.nml -o ${path_restart}/${i} -p ctrl.run_step="spinup_ismip6" tf_cor.name="dT_nl" marine_shelf.gamma_quad_nl=14500 ytopo.kt=1.0e-3 ymat.enh_shear=1 ytopo.bmb_gl_method=${i}
...
for k in $exps; do
    for i in $bmbmeth; do
        for j in $timesteps; do
            ...
            ./runylmox ${runopt} -e ismip6 -n par/yelmo_ismip6_Antarctica.nml -o ${fldr}/${k}_${i}_dtt${j} -p abumip.scenario=${k} ctrl.run_step="abumip_proj" yelmo.restart=${file_restart} abumip_proj.scenario="ctrl" tf_cor.name="dT_nl" marine_shelf.gamma_quad_nl=14500 isostasy.method=0 ${paropt} ydyn.ssa_lat_bc="floating" ytopo.bmb_gl_method=${i} abumip_proj.dtt=${j}
