...
if [ ${floating} -eq 1 ]; then
./runylmox ${runopt} -e ismip6 -n par/${namelist} -o ${fldr}/spinup_${exp_name}_fd${i} -p ctrl.run_step="spinup_ismip6" tf_cor.name="dT_nl" marine_shelf.gamma_quad_nl=14500 ytopo.kt=1.0e-3 ymat.enh_shear=1 ydyn.ssa_lat_bc="floating" yelmo.dt_method=0 yelmo.pc_n_redo=1 yelmo.log_timestep=.true. spinup_ismip6.dtt=${i} yelmo.dt_min=${min_fixed_dtt}
fi
if [ ${marine} -eq 1 ]; then
./runylmox ${runopt} -e ismip6 -n par/${namelist} -o ${fldr}/spinup_${exp_name}_md${i} -p ctrl.run_step="spinup_ismip6" tf_cor.name="dT_nl" marine_shelf.gamma_quad_nl=14500 ytopo.kt=1.0e-3 ymat.enh_shear=1 ydyn.ssa_lat_bc="marine" yelmo.dt_method=0 yelmo.pc_n_redo=1 yelmo.log_timestep=.true. spinup_ismip6.dtt=${i} yelmo.dt_min=${min_fixed_dtt}
fi
